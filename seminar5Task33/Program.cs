...
while (!endApp)
{
    ...
    int indexElement = FindElementInArray(randomNumbersArray, number);

    PrintResultToConsole(
        (indexElement > 0)
            ? $"Число {number} находится в массиве под индексом {indexElement}"
            : $" Числа {number} в массиве нет "
...
//Метод возвращает индекс первого найденного элемента массива
// если элемент не найден вернёт -1  
int FindElementInArray(int[] array, int element)
{
    int elementIndex = -1;
