//=============================================================================
//                       Задача 23* (вывести числа друг над другом)
...
// 1 элемент - это  строка из кубов чисел от 1 до N
string[] GetCubesOfN(int number)
{
    ...
    {
        string cube = ((int)Math.Pow(i, 3)).ToString();

        int count =
            ((int)Math.Pow(i, 3)).ToString().Length - i.ToString().Length;

        string spaces = new String(' ', count);

        numbersString += i.ToString() + " " + spaces;
        cubesString += cube + " ";
    }
